...
class JumboCog(commands.Cog):
    ...
    async def jumbo_start(self, interaction: discord.Interaction, title: str, description: str, deadline: str):
        ...
        await self.bot.db.jumbo_set_config(guild_id, title, description, deadline_dt)
        ...
        view = JumboBuyView(self.bot, self.bot.db, guild_id)
        ...
        await self.bot.db.jumbo_set_panel_message(
            guild_id=guild_id,
        ...
        for rank, match_len in RANK_RULES.items():
            ...
            for e in candidates:
                ...
                if strict_hit(winning, number, match_len):
                    ...
                    results[rank].append({"user_id": e["user_id"], "number": number})
    ...
    async def jumbo_set_prize(self, interaction: discord.Interaction, winning_number: str):
        ...
        await self.bot.db.jumbo_set_winning_number(str(interaction.guild.id), winning_number)
    ...
    async def jumbo_my_numbers(self, interaction: discord.Interaction, search: str | None = None):
        ...
        rows = await self.bot.db.jumbo_get_user_numbers(guild_id, user_id)
    ...
    async def jumbo_reset(self, interaction: discord.Interaction):
        ...
        await self.bot.db.jumbo_clear_entries(str(interaction.guild.id))
        await self.bot.db.jumbo_clear_winners(str(interaction.guild.id))
